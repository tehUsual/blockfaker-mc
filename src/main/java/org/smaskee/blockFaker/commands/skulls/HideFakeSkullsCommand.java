...
public class HideFakeSkullsCommand extends BaseCommand implements TabCompleter {
    private final SkullSender skullSender;

    public HideFakeSkullsCommand(JavaPlugin plugin) {
        super(plugin);
    ...
    protected boolean execute(CommandSender sender, Command command, String[] args) {
        ...
        for (int i = 1; i < args.length; i++) {
            ...
            if (fakeSkull != null) {
                ...
                dataManager.setSkullVisibility(args[i], player.getPlayerId(), false);
            }
        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            ...
            List<String> usedSkullNames = Arrays.asList(Arrays.copyOfRange(args, 1, args.length - 1));
            completions.addAll(dataManager.getAllSkulls().keySet().stream()
                    .filter(blockName -> !usedSkullNames.contains(blockName))
                    .toList());
        }
