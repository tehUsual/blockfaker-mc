...
public class ShowFakeBlocksCommand extends BaseCommand implements TabCompleter {
    private final BlockSender blockSender;

    public ShowFakeBlocksCommand(JavaPlugin plugin) {
        super(plugin);
    ...
    protected boolean execute(CommandSender sender, Command command, String[] args) {
        ...
        for (int i = 1; i < args.length; i++) {
            ...
            if (fakeBlock != null) {
                ...
                dataManager.setBlockVisibility(args[i], player.getPlayerId(), true);
            }
        }

        if (player.isOnline()) {
            blockSender.sendMultipleBlocks(player.getPlayer(), fakeBlocks.toArray(new FakeBlock[0]), true);
        }
